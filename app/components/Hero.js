...
export default function Hero() {
  ...
  useEffect(() => {
    ...
    const animateBars = () => {
      if (audioPlaying) {
        setSoundBars((prevBars) => prevBars.map(() => (audioPlaying ? Math.random() * 0.8 + 0.2 : 0.2)))
      }
      animationFrame = requestAnimationFrame(animateBars)
    }
          ...
          >
            {soundBars.map((height, index) => (
              <motion.div
                ...
                animate={{
                  height: audioPlaying ? [`${height * 60}px`, `${Math.random() * 60 + 10}px`] : `${height * 20}px`,
                  opacity: audioPlaying ? [0.7, 1] : 0.4,
                }}
                transition={{
                  duration: audioPlaying ? 0.4 : 0.8,
                  repeat: audioPlaying ? Number.POSITIVE_INFINITY : 0,
                  repeatType: "reverse",
                }}
              />
            ))}
          ...
          >
            ...
            >
              ...
              <motion.div
                ...
                layoutId="cta-button-bg"
              />
              <motion.span className="absolute inset-0 z-0 bg-white group-hover:bg-transparent transition-colors duration-300" />
            </motion.a>
