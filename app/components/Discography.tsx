...
export default function Discography() {
    ...
    useEffect(() => {
        async function fetchTracks() {
            try {
                ...
                const formattedTracks = await Promise.all(data.tracks.slice(0, 6).map(async (track: any, index: number) => {
                    try {
    ...
    useEffect(() => {
        ...
        if (hoveredTrack && hoveredTrack.previewUrl && audio) {
            audio.src = hoveredTrack.previewUrl;
...
function getGradientByGenre(genre: string) {
    switch (genre) {
        case 'electronic':
...
function getPatternByGenre(genre: string) {
    switch (genre) {
        case 'electronic':
